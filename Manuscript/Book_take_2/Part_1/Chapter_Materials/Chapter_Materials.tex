...
for accounting material flows and accumulations within economies.
...
in the goods produced by the sector ($\dot{P}$)\nomenclature[Pb]{$\dot{P}$}{product mass flow rate [kg/s]}, which leave from the right,
...
upon the accumulation\index{accumulation} 
...
% years the BEA has tried to fix this problem (of
...
when do we begin and end counting and where is our system
boundary (control volume). We saw that some things that we count (apples) can be
