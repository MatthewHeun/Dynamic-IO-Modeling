...
for accounting material flows and accumulations\index{accumulation!of materials}
within economies.
...
of material accumulations\index{accumulation!of materials}
within an economy. 
...
resources\index{natural resources}, 
...
materials\index{accumulation!of materials} within an economy, 
...
the accumulation\index{accumulation!of apples} rate.
...
leave the house as waste\index{waste} (in the apple core), 
...
must go somewhere, 
whether it stays within the volume (accumulation\index{accumulation}) 
...
of the First Law\index{First Law of Thermodynamics}.
...
or discharge of waste\index{waste} material or energy 
\emph{from} the economy
occurs as an interaction between 
the economy and the biosphere\index{biosphere}.
...
products ($P$), resources\index{resources} ($R$),
short-lived goods ($S$),
...
except for some proportion that is wasted\index{waste}.
...
In this framework, 
resource materials are not accumulated\index{accumulation!of materials} within a sector.
...
of these materials\index{accumulation!of materials} within the sector. 
...
or as capital goods\index{capital goods} ($\dot{K}$).
...
with the biosphere\index{biosphere} (0) as depicted in
...
biosphere\index{biosphere} (0).\footnote{Double 
...
capital goods\index{capital goods} ($\dot{K}_{11}$)
...
% of capitalindex{accumulation!of capital}
...
of natural capital\index{accumulation!of natural capital}~($R_{0}$ and $S_{0}$) 
...
from naturally occurring accumulations\index{accumulation!of natural resources} that are 
far from equilibrium with their surroundings,
e.g.\ fossil fuel\index{fossil fuel} reservoirs or seams of high-grade ore. 
...
accumulation of capital\index{accumulation!of capital} in society~($K_{1}$)
...
in terms of the accumulation\index{accumulation!of natural resources} 
(or more accurately, depletion)
...
flows.
%
% Next sentence is awkward and convoluted.
% 
...
% ``consumer durables,'' % chktex 38
% though also includes other products, 
% such as housing, 
% that economists might not include in that category.
% \footnote{
% %For national accounting, 
% % capital goods are used only within 
% % productive sectors of the economy.
% %%all capital goods are produced and used within the industry sectors. 
% %%Bridges, roads, the internet, and all other public infrastructure are counted as 
% %%government spending (ie. consumption), not investment in capital.
% **** Mik - I took this footnote out altogether because in the last two
% years the BEA\index{Bureau of Economic Analysis} has tried to fix this problem (of
% categorizing all investment in the private sector) through adding
% addenda onto the tables, etc. There is not a clear
% sentence to write here without a lot of caveats. ****}
%
% This is the fix that Becky and Matt developed.
%
One point worth noting is that 
our flow of 
``capital goods''\index{capital goods}
...
of man-made capital\index{accumulation!of capital}~($K$) in each sector ($j$)
...
%building up capital stock
%in the production sector
%$\left(\frac{\mathrm{d}K_{2}}{\mathrm{d}t}\right)$;
%manufacturing goods for final consumption
%$\left(\dot{R}_{21} + \dot{S}_{21} + \dot{K}_{21}\right)$;
%providing short-lived goods to support the 
%production sector ($\dot{S}_{22}$); and
%overcoming depreciation of production 
...
resources\index{accumulation!of resources}~(R) 
or short-lived capital goods\index{accumulation!of short-lived capital}~(S) 
in economic sectors,
then we may say:
...
the accumulation of man-made capital stock\index{accumulation!of capital}~($K_{j}$)
...
Data on the actual flows %**** What do you mean by ``rest of the flows?''
...
of materials\index{accumulation!of materials}.
