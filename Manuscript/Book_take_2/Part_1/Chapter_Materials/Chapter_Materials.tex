...
society. % **** Need to show a bit more where these equations are coming from. I don't get it. --MKH ****

Firstly,
noticing that the right-hand side
of Equation~\ref{eq:A_CV_0c} is the negation 
of the right-hand side of Equation~\ref{eq:A_CV_1c},
we may substitute Equation~\ref{eq:A_CV_0c} into 
Equation~\ref{eq:A_CV_1c} to obtain:
...
We may now combine Equation~\ref{eq:A_K_11} with
...
multiplied by some depreciation rate ($\gamma_{K}$) in units of
inverse time (e.g.\ 1/year), such that we may re-write
Equation~\ref{eq:A_dK_1/dt} as:

\begin{equation} \label{eq:A_dK_1/dt_b}
	\frac{\mathrm{d}K_{1}}{\mathrm{d}t}
	= - \left(
	\frac{\mathrm{d}R_0}{\mathrm{d}t}		
	+	\frac{\mathrm{d}S_0}{\mathrm{d}t}
...
Substituting
Equations~\ref{eq:B_R0}-\ref{eq:B_P2_def}
into Equations~\ref{eq:B_CV_0}-\ref{eq:B_CV_2}, 
...
Combining and simplifying these three equations we arrive at the result:
