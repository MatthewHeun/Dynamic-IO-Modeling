...
In this appendix, we show that the EI-O method 
accounts for the infinite recursion of energy
demands for production.

The single-sector economy 
of Figures~\ref{fig:B_materials},~\ref{fig:B_energy},~\ref{fig:B_total_energy},
and~\ref{fig:B_value}
...
If we consider the Biosphere (0) to be producing
a valuable product, namely energy, we can say
...
\begin{equation}
	a_{02} \equiv \frac{\dot{E}_{02}}{\dot{X}_{2}}.
\end{equation}
...
The sum of all direct energy required for the economy 
to produce at a rate of $\dot{X}_{2}$ ($\dot{E}_{demand,tot}$) is an infinite sum.
...
\begin{equation} \label{eq:epsilon_process_SS_intermediate}
	\varepsilon_{2} 
	= \frac{\dot{E}_{demand,tot}}{\dot{X}_{2}} 
	= a_{02}(1 + a_{22} + a_{22}^2 + \ldots) 
	= a_{02}\sum_{n=0}^{\infty}a_{22}^{n}.
\end{equation}

Realizing that 
$\sum_{n=0}^{\infty}a_{22}^{n} 
= \frac{1}{1-a_{22}}$ 
and 
$a_{02} 
= \frac{\dot{E}_{02}}{\dot{X}_{2}}$ 
gives
...
Accounting for the differences between scalar and matrix equations 
and neglecting energy flows from society to the economy ($\dot{T}_{12} = 0$), 
accumulation of embodied energy\index{accumulation!of embodied energy} in the economy
$\left( \frac{\mathrm{d}B_{2}}{\mathrm{d}t} = 0 \right)$,
and physical depreciation $\left( \gamma_{B_{2}}B_{2} = 0 \right)$,
\index{depreciation}
Equations~\ref{eq:epsilon_leontief_with_A} and~\ref{eq:epsilon_process_SS} are identical, 
indicating that the EI-O approach accounts 
