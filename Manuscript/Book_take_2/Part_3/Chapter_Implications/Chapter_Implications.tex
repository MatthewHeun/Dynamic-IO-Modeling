...
capital stock is addressed in Section~\ref{sec:XXXX}
...
\begin{itemize}
	...
	capital stocks~($\sum\limits_{j}\gamma_{K_{j}}K_{j}$) **** MCD - can j
	subscript on $\sum$ be put underneath. ****
\end{itemize}

Assuming,
first,
that a steady-state economy exists when there is the level
of capital stock remains constant
$\left(\sum\limits_{j}\frac{\mathrm{d}K_{j}}{\mathrm{d}t}=0\right)$
is met,\footnote{Note
that,
...
As such,
the steady-state does not proclude development of the economy,
...
sustainably,
then **** MCD - right out maths in words ****
$\frac{\mathrm{d}R_{0}}{\mathrm{d}t}$ had
better be at some rate lower than the biosphere
can replenish the natural capital stocks.

Secondly,
both $\sum_{i,j}\dot{S}_{ij}$
and $\sum_{j}\gamma_{K_{j}}K_{j}$
may be at high or low levels with the
economy still operating at steady state.
...
and napkins)
is large.
**** MCD - define relationship between $\gamma$
and average lifetime. Capital stock may be constant
but we are still unsustainable. ****
Additionally,
if the average lifetime of capital goods such
as consumer electronics (cell phones,
tablets,
notepads)
decreases,
this causes an increase in the need for flows of
replacement materials.
In the absense of extreme recycling of materials,
...
As such,
a larger accumulation of capital stock requires
greater draw on natural resources and is thus harder
to maintain within any sustainability constraint.

**** MCD - bring in Daly here ****
Characterizing a steady-state economy as one
with a constant material through-put **** MCD - explain in words.
Be more explicit about throughput as input or output. 
We are looking at withdrawl from the biosphere (input).  
Recast as extraction (defined by equation) and then also
waste. ****,
could be defined within our framework
by the constraint:

\begin{equation}
	\frac{\mathrm{d}\dot{R}_{0}}{\mathrm{d}t} 
	= \frac{\mathrm{d}\dot{S}_{0}}{\mathrm{d}t}
	= 0.
\end{equation}
 **** MCD - refer to Figure here ****
Within this constraint,
more effective use of materials could
theoretically allow increasing accumulation
of man-made capital ($K$) 
and increasing flow of short-lived goods
...
**** MCD - Discuss short-lived stuff more ****

**** MCD - GDP discussion can refer to Section 7.2 ****
