...
$\left( \left. \frac{\mathrm{d}\vec{B}_{K}}{\mathrm{d}t} \right|_{\mathrm{other}} = 0 \right)$}
%+++++++++
...
**** Matt needs to complete this discussion. ****
...
we see that the energy demands ($\vec{E}_{0}$) must be calculated differently. 
...
that embodied energy ($\vec{B}_{K}$) is a \emph{stock}, not a flow.
...
to increase.
But, there is no expectation that $GDP$ and $\vec{B}_{K}$ move 
...
the stock of energy embodied in the economy ($\vec{B}_{K}$) may decrease while
$GDP$ increases due to the flow of ``aid'' into an economy.
**** Probably need a better example here.
I'm not sure that post-colonial Africa is correct, 
because much of the ``aid'' goes for infrastructure.
Are there other examples? --MKH ****
...
For capital~($\vec{B}_{K}$) to be useful, we need financial resources
or currency~($\dot{X}$) to purchase 
direct energy~($\dot{E}$) to power the capital,
resources~($\dot{R}$) to feed the capital, and
workers~(as represented by societal energy input to the economy,~$\vec{T}_{1}$) 
to operate the capital.
Each of these factors alone ($\vec{B}_{K}$, $\dot{E}$, $\dot{R}$, and $\vec{T}_{1}$)
...
thereby reducing $\hat{\bm{\gamma}}_{K}$, 
will slow the effect of depreciation 
$\left(\hat{\bm{\gamma}}_{K} \vec{B}_{K} \right)$ 
...
from decrease in $\hat{\vec{\gamma}}_{K}$ and 
the downward pressure on growth from net reduction of $\vec{E}_{0}$ 
offset each other, 
the growth rate $\left(\frac{\mathrm{d}\vec{B}_{K}}{\mathrm{d}t}\right)$ 
...
no change in growth $\left(\frac{\mathrm{d}\vec{B}_{K}}{\mathrm{d}t}\right)$.
