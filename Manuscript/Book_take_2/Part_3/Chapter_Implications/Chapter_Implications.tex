...
In this section, we unpack the (often unstated) assumptions from the literature
and develop an expression that allows translation
between our analysis and the typical analysis in the literature. 
...
the literature is implicitly assuming that 
...
\noindent{}Comparison with Equation~\ref{eq:aij_def_expanded}
indicates that the literature neglects capital stock flows 
($\dot{X}_{\dot{K}}$).\footnote{Variables written with a ``prime''
(e.g.\ $a^{'}$) indicate definitions and terms as used in the literature.}
Thus, the the input-output matrix 
($\vec{A^{'}}$, compare to Equation~\ref{eq:A_matrix_def}) is
...
**** MCD---need to compare these three formulations ****
...
\begin{align}\label{eq:const_throughput}
	...
	\sum\limits_{i}
			\left(
				\frac{\mathrm{d}\dot{R}_{i0}}{\mathrm{d}t}
				+ \frac{\mathrm{d}\dot{S}_{i0}}{\mathrm{d}t}
				+ \frac{\mathrm{d}\dot{K}_{0}}{\mathrm{d}t}
			\right)																			&
	= 0.
\end{align}
...
**** MCD---maybe change back to d/dt formulation 
and split into multiple equations. 
Need to eliminate the appearance that $\dot{R}$ and $\dot{S}$ flows are equivalent. ****
Matt, you can have final ruling on this, but I think the double dot notation looks better, MCD ****
...
**** MCD---need to compare these three ****
...
\begin{equation}\label{eq:const_GDP}
	\frac{\mathrm{d}}{\mathrm{d}t}GDP
	=	\sum\limits_{j}\frac{\mathrm{d}}{\mathrm{d}t}\dot{X}_{j}
	= 0
\end{equation}
...
$\dot{S}_{0}$, it is **** carry on discussion---MCD ****
