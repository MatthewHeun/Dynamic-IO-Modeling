...
from the biosphere, typically energy-producing sectors (called the DIRECT method) and 
(b)~the points of conversion to useful work, typically all energy \emph{consuming} sectors
(called the DEC method, Direct Energy Conversion). 
...
There is debate in the literature about whether government and households 
...
Costanza~\cite{Costanza:1980ww} showed that energy intensity results
are a function of boundary selection. 
...
\noindent{}wherein ``final demand'' has been endogenized
...
and the vectors and matrices of Equations~\ref{eq:B_vec_def}--\ref{eq:gamma_hat_matrix_def}
and~\ref{eq:A_matrix_def} have been extended to include ``final consumption,'' sector (1).
...
did not include the solar to the economy.
Whether solar input to the economy 
should be considered is probably dependent upon 
the objectives of the analysis. 

The motivation for this book is primarily 
the effects of declining energy resource quality in industrial economies 
...
An approach that fully-integrates solar energy 
