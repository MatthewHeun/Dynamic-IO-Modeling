...
And, they still are. 
To this day, US national accounts do not include 
...
is built upon the clockwork metaphor and traditional model for the economy
...
and larger food requirements per unit mass.%
	...
	%
	\begin{equation}
		\frac{\dot{Q}}{m} = q_{0} m^{-1/4} \; ,
	\end{equation}
	%
	from which it can be seen that smaller organisms (smaller $m$) consume more
	energy per unit mass~($\dot{Q}/m$).
	}
...
The SEEA allows national governments to answer questions
...
% We believe it important to  generate a very explicit set of guidleines
% as to how to undertake a biophysical analysis of an economy or some portion of that.
% Consequently the next x chapters develop the methods step
% by using as much as possible the same diagrams
% and the same type of equations to allow for, as much as possible,
% a step by step understanding our approach.
...
using, as much as possible, the same model structure for each.
