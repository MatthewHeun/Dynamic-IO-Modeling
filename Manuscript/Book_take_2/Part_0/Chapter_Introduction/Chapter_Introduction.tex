...
It is easy to collect data that confirms our models,
because the models tell us which data to collect.
It takes courage to collect data that might be counterfactual
and still more courage to modify or let go of established models and metaphors.

For many decades, the ``economy is a machine'' metaphor
...
**** MCD---not sure why the reference 
is not correct ****
...
Although the biosphere is represented here in
Figure~\ref{fig:perp_motion_1}, 
...
Since the economy is independent of any other processes
...
we are delivered to a moral imperative for economic growth.
...
Since Figure~\ref{fig:perp_motion_1} has no flow of energy
to the economy,
...
from where it takes in oxygen and to the digestive
system where it takes in processed food
which is passed to the cells.
...
The economy is presented as a ``perpetual motion machine,'' % chktex 38
seemingly able to operate indefinitely with 
no binding constraints.


The traditional, economy-is-all viewpoint fought back
by appeals to \emph{substitutability}.
A particular material (oil) may be in short supply,
but our technological expertise will allow us to substitute
...
the larder of the economy, if you will.\cite{Norgaard2010}
...
an anomaly to the normal functioning of the economy.~\cite{XXXX}
...
High quality (low entropy) resources come in,
low quality (high entropy) resources come out.
...
The 2007 global financial crisis is an example of one such systemic event. 
**** BRH cite the economist article ***
The 1930's dust bowl is also a result of a systemic event. 
...
implemented in a new, and unknown ecosystem. 
***BRH cite Lockertz 1978 ***
...
to form an energy input-output (EI-O) model
...
into the biosphere
rather than on resource depletion.~\cite{Lenzen1998, 
ConAccount1998, Hoekstra2003, Bailey2008, Pedersen2006, Turner2007}
...
%	or increase consumption.}
	...
	**** MCD---I did not include these here. Have we included them elsewhere?****
\end{itemize}
...
to be transformed into final products (shirts) and \emph{funds}---stocks
...
and \emph{efficient causes}.~\cite{Daly2002}
...
Parts~\ref{part:matter}~and~\ref{part:value}.
draws out some of the implications
