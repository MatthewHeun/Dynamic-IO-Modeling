...
the System of Environmental-Economic Accounting (SEEA)~\cite{UNSEEA:aa} is in its third revision 
...
flows to and from the biosphere.} ****define subjective theory of value ***
...
is used for final consumption (NIPA categories, ``CIGX''), including Private Fixed Investment ($\dot{K}$). However, the more detailed, but less accurate, Fixed Asset accounts are
used to distinguish between the capital that is produced in the automobile industry
that goes to self-use from that which is final consumption.
...
The value of capital flows from other sectors into the automobile industry, ($\dot{K}$) is 
also obtained from the Fixed Assets BEA table.  The ``waste output to the environment''
can be approximated by the value of the commodities that are ``made'' in all industries 
and ``used'' by the ``Waste Management Sector'' (NAICS 56).  Table~\ref{tab:data} contains the sources from the BEA that were used to obtain the values in Figure~\ref{fig:PERKS_value_auto_ind}.\footnote{See Appendix~\ref{chap:auto_value_flows} for detailed calculations.}
...
&&\\
    ...
    Resources \cr (self-use) &  139,259 &  KLEMS Intermediate Use Estimates 2011     \\
&&\\
    Output to Waste Sector & 441  &  KLEMS Intermediate Use Estimates 2011     \\
&&\\
    Capital &  46,332  &  Fixed Assets 2011 (non-residential detailed estimates)     \\  
&&\\
    Capital (self-use) & 15,181 & Fixed Assets 2011 (non-residential detailed estimates)      \\
&&\\
    Gross Economic Output & 474,186  &  The 2011 Use of Commodities by Industries \\

&&\\
    Net Economic Output & 319,746   &  The 2011 Use of Commodities by Industries \\
    \bottomrule
  \end{tabular}
