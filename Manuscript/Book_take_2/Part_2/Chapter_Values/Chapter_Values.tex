...
%%%%%%%%%%
The theoretical model of energy and resource flows through an economy can be empirically tested using U.S. data available 
...
are the Input-Output accounts (IO) and the Capital Flow Table (CFT). 
 The IO table tracks commodities that are made in one industry and then used in another – intermediate inputs. 
The “making” industries related to extraction, refining, and utilities 
mark the entry point for natural resources and energy into the economy (the “red” and “green” lines in the model). 
 These coincide with the first three industries in the NAICS (North American Industry Classification System)
 that the BEA (Bureau of Economic Analysis) uses to track economic information. 
****BRH  add cite (Lawson and et al. 2002, 25). **** Thus, the majority of the data needed to calculate the dollar value of the flow of energy & natural resources through the economy is publicly available from the BEA website. 

Using these data, Figure \ref{fig:PERKS_value_auto_ind} provides estimates of value flows for one-sector in an economy, the auto industry.
...
This maps the outputs of construction and certain manufacturing industries to all other industries. ****BRH add cite (Bonds and Aylor 1998)****  The “waste output to the environment” can be approximated by the value of the commodities that are “made” in all industries and “used” by the “Waste Management Sector (NAICS 56).
Using the automotive industry as an example, the KLEMS, Input-Output, and Capital Flow data\footnote{Available from http://bea.gov/iTable/index\_industry.cfm.} could be used to estimate the model as demonstrated in Figure~\ref{fig:Values_data_sources}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\linewidth]{Part_2/Chapter_Values/images/Values_data_sources.pdf}
\caption[Data Sources for Auto Industry (IOC 3361MV) Example]{Data Sources for Auto Industry (IOC 3361MV) Example.}
\label{fig:Values_data_sources}
\end{figure}
