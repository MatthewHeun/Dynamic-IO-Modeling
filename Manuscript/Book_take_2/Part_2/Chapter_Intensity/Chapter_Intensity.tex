...
\caption[System boundary for process analysis (dashed oval) and I-O analysis (solid box).]{System boundary for process and I-O analyses, adapted from~\cite{Bullard:1978vd}.}
...
\caption[Top-down vs.\ bottom-up analyses]{Advantages (pros) and disadvantages (cons) of ``top-down,'' I-O and ``bottom-up,'' process-based analyses.  %chktex 38
Adapted from~\cite{Hendrickson2006}.} 
...
%%%%%%%%%% Methodology %%%%%%%%%%
\section{Methodology}
\label{sec:intensity_methodology}
%%%%%%%%%%
...
\begin{itemize}
	...
	\item{$\dot{E}_{20} = 0$ meaning that $\dot{T}_{20} = \dot{B}_{20}$, 
		because direct energy is not wasted to the biosphere at any significant rate,%
			\footnote{  %chktex 42
			Oil spills and gas leaks notwithstanding. 
...
we recommended a physical accounting framework.
...
In Chapter~\ref{chap:summary}, we discuss further the need for additional data.
