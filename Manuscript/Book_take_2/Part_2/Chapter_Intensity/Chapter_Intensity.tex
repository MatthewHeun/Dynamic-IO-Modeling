...
This definition 
of energy intensity (Equation~\ref{eq:epsilon_output_def_g_and_s}) 
includes the embodied energy of products in the $\dot{T}_{j}$ term. 
A narrower definition would be 
...
because $\frac{\mathrm{d}E_i}{\mathrm{d}t} = 0$, because direct energy
does not accumulate within economic sectors and
(b) $\dot{T}_{0j} = \dot{E}_{0j}$ because $\dot{B}_{0j}~=~0$, 
...
from Beyer~\cite[Formula 6.2, p. 308]{Beyer:1991vd}
