...
We define the input-output ratio\index{input-output ratio} ($a_{ij}$)
...
Input-output ratios\index{input-output ratio} ($a_{ij}$) are given in mixed units, 
...
ratios\index{input-output ratio} ($a$).}
...
\begin{equation} \label{eq:A_matrix_def}
	...
	\begin{bmatrix}
		...
		a_{32} & a_{33}	
	\end{bmatrix}.
\end{equation}\index{input-output matrix}
...
and input-output ratios\index{input-output ratio} ($\vec{A}$).
...
estimate the embodied energy accumulation rate\index{accumulation!of embodied energy}
...
sector input-output ratios\index{input-output ratio} ($\vec{A}$), 
...
of outputs from all 87 BEA\index{Bureau of Economic Analysis} sectors.
...
of the differences between DIRECT and DEC\index{direct energy conversion} methods
...
(NAICS\index{North American Industry Classification System} sector 336111) generates
...
sector (NAICS\index{North American Industry Classification System} 
33611x) energy intensity values.\cite{EIOLCA2014}}
\begin{center}
\begin{tabular} {r @{\hspace{2em}} l}
	\toprule
	Year & Energy Intensity [kJ/\$] \\
...
for a vector of energy intensities for the entire economy.
We then reviewed several studies of energy intensity in the literature 
...
from the framework presented in
