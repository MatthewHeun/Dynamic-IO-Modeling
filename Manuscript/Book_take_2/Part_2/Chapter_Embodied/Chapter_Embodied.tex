...
into Equations~\ref{eq:A_total_energy_1} and \ref{eq:A_total_energy_2} yields
...
term $-\dot{Q}_{21}$, which appears different from 
Equation~\ref{eq:embodied_energy_accounting_2}, which
...
Furthermore, we note that $\dot{E}_{21} = 0$, 
because society does not supply direct energy 
to the biosphere. We can simplify 
Equations~\ref{eq:A_dB1/dt} and \ref{eq:A_dB2/dt} as
...
(Equations~\ref{eq:CV_dB_1}--\ref{eq:CV_dB_3}) to obtain
