...
\noindent Figure~\ref{fig:embodied_single_producer} shows that
...
The waste energy term ($\dot{Q}_{out}$) 
in Equation~\ref{eq:embodied_energy_accounting}
is an \emph{outflow} of energy from the sector. 
The embodied energy 
...
Equation~\ref{eq:embodied_energy_accounting_3} shows that waste 
energy \emph{out} of an economic sector ($\dot{Q}_{out}$) 
is \emph{additive to} energy embodied
in the products of the sector ($\dot{B}_{out}$).
It also shows that embodied energy accumulating
within the sector 
$\left( \mathrm{when} \; \frac{\mathrm{d}B}{\mathrm{d}t} > 0 \right)$
reduces the embodied energy output from the sector
($\dot{B}_{out}$), as expected.

We now apply the embodied energy accounting
equations to each of our example economies.
...
A total energy accounting around the single-sector economy (2) gives
...
We now turn to Example B, a two-sector economy.
